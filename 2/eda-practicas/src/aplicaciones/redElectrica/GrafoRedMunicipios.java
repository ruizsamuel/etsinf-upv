...
public class GrafoRedMunicipios {
    ...
    public final static String DIR_FICHEROS = "src" + File.separator + "aplicaciones" + File.separator
    ...
    public double crearAdyKruskal() {      
        ...
        // COMPLETAR
        adyKruskal = new TablaHash<Municipio, ListaConPI<Municipio>>(gRM.numVertices());
        double res = 0.0;
        for(int i = 0; i< k.length; i++){
            res += k[i].getPeso();
            int o = k[i].getOrigen();
            int d = k[i].getDestino();
            Municipio origen = verticesAMunicipios.recuperar(o);
            Municipio destino = verticesAMunicipios.recuperar(d);
            if(adyKruskal.recuperar(origen) == null){
                ListaConPI<Municipio> e = new LEGListaConPI<Municipio>();
                e.insertar(destino);
                adyKruskal.insertar(origen, e);
            }else{
                ListaConPI<Municipio> e = adyKruskal.recuperar(origen);
                e.insertar(destino);
            }
        }
