...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        ...
        while (!q.esVacia()) {
            ...
            for (l.inicio(); !l.esFin(); l.siguiente()) {
                Adyacente a = l.recuperar(); 
                if (visitados[a.destino] == 0) {
                    res[ordenVisita++] = new Arista(origen, a.destino, a.peso);
                    visitados[a.destino] = 1;
                    q.encolar(a.destino);
                }
            }  
    ...
    public Arista[] kruskal() {    
        Arista[] a = new Arista[numVertices() -1];
        int cardinalE = 0;
        ...
        for(int i = 0; i < numVertices(); i++){
            ...
            for(adyacentes.inicio(); !adyacentes.esFin(); adyacentes.siguiente()){
                Adyacente l = adyacentes.recuperar();
                Arista arista = new Arista(i, l.getDestino(), l.getPeso());
                aristasFactibles.insertar(arista);
            }
        }
        while(cardinalE < numVertices() -1 && !aristasFactibles.esVacia()){
            Arista aris = aristasFactibles.eliminarMin();
            int ccV = cc.find(aris.getOrigen());
            int ccW = cc.find(aris.getDestino());
            if(ccV != ccW){
                a[cardinalE++] = aris;
                cc.union(ccV, ccW);
            }
        }
        return (cardinalE == numVertices()-1)  ? a : null;
    }
}
