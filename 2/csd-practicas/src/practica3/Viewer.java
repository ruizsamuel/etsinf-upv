...
public class Viewer extends Panel {
    ...
    void drawDir(Graphics g, int x, int y, Dir d) {
        switch (d) {
            case N -> g.fillOval(21 + 50 * y, 30 + 50 * x, 8, 8);
            case W -> g.fillOval(50 * y, 51 + 50 * x, 8, 8);
            case S -> g.fillOval(21 + 50 * y, 72 + 50 * x, 8, 8);
            case E -> g.fillOval(42 + 50 * y, 51 + 50 * x, 8, 8);
        }
    }
    ...
    public void paint(Graphics g) {
       ...
       g.setColor(Color.GRAY); g.setFont(plain);
        ...
        for (int i=0; i<=idx; i++) {
            Op op=ops[i];
            try {
            if (Class.forName("practica3.Hi"    ).isInstance(op)) {mat.opHi     (op.a,((Hi)op).p);}
            if (Class.forName("practica3.Bye"   ).isInstance(op)) {mat.opBye     (op.a);}
            if (Class.forName("practica3.Turn"  ).isInstance(op)) {mat.opTurn     (op.a,((Turn)op).d);}
            if (Class.forName("practica3.Go"    ).isInstance(op)) {mat.opGo     (op.a);}
            if (Class.forName("practica3.ChgDir").isInstance(op)) {mat.opChgDir(op.a,((ChgDir)op).d);}
