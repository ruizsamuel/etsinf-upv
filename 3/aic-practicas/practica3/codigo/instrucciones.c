...
void color_instruccion(char *color,
        dword PC
) {
    static char *paleta[NUM_STYLES] = {
            "darkred", "darkcyan", "darkblue", "darkolivegreen", "darkorange", "deeppink", "royalblue", "brown",
            "magenta", "blue", "darkslategray", "forestgreen", "darkviolet", "mediumblue", "steelblue", "darkorchid"
    };

    //int index = bloque_instruccion(PC);
    int index = (PC/4)%NUM_STYLES;
...
void imprime_instruccion_color(
    ...
    if (PC == -1) {
        ...
        if (en_region(mmem, PC, r_text)) {
            instr = lee_mem_instruc(mmem, PC);

            if (strlen(instr.texto) == 0) {
                ...
                fn_riscv_decode(PC, instr.valor, &fmt, &opcode, &rs1, &rs2, &rs3, &rd, &rm, &imm,
                                nemotecnico, instr.texto);
            }

            if (encolor == SI) {
                color_instruccion(color, PC);
                sprintf(instruc, "<font color=\"%s\">%s</font>", color, instr.texto);
            } else {
                sprintf(instruc, "%s", instr.texto);
            }
        } else {
 ...
 */
void muestra_campo(
        g_string_t *binario, g_string_t *campos,
        uword dump, modo_formato_t format_mode,
    ...
    switch (format_mode) {
        case format_full:
            g_str_printf(binario, g_mode_concat,
                         "<span style='color:%s' title='%s'>%s</font>", color, texto, aux);
            if (n_bits == 1) {
                g_str_printf(campos, g_mode_concat,
                             "<span class='field' style='color: %s; border-color: %s; width: %dpx' title='%s'>%c</span>",
                             color, color, n_bits * BITSIZE_FULL, texto, texto[0]);
            } else {
                g_str_printf(campos, g_mode_concat,
                             "<span class='field' style='color: %s; border-color: %s; width: %dpx' title='%s'>%s</span>",
                             color, color, n_bits * BITSIZE_FULL, texto, texto);
        ...
        case format_compact:
            g_str_printf(campos, g_mode_concat,
                         "<span class='field' style='color: %s; border-color: %s; width: %dpx' title='%s'>%s</span>",
                         color, color, n_bits * BITSIZE_COMPACT, texto, aux);
            break;
        default:;
    }
...
void muestra_formato(
    ...
    switch (formato) {
        case FormatoR:
            g_str_copy(&aux2, g_mode_begin, "R ");
            muestra_campo(&aux, &aux2, dump, format_mode, "f7", COL_F7, 31, 25);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs2", COL_RS2, 24, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;
        case FormatoR4rm:
            g_str_copy(&aux2, g_mode_begin, "R ");
            muestra_campo(&aux, &aux2, dump, format_mode, "rs3", COL_RS3, 31, 27);
            muestra_campo(&aux, &aux2, dump, format_mode, "f7", COL_F7, 26, 25);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs2", COL_RS2, 24, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "rm", COL_RM, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;
        case FormatoR3rm:
        case FormatoR2rm:
            ...
            break;
        case FormatoI:
            g_str_copy(&aux2, g_mode_begin, "I ");
            muestra_campo(&aux, &aux2, dump, format_mode, "imm", COL_IMM, 31, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
        ...
        case FormatoIshift:
            g_str_copy(&aux2, g_mode_begin, "I ");
            muestra_campo(&aux, &aux2, dump, format_mode, "f6", COL_F7, 31, 26);
            muestra_campo(&aux, &aux2, dump, format_mode, "shamt", COL_IMM, 25, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;
        case FormatoS:
            g_str_copy(&aux2, g_mode_begin, "S ");
            muestra_campo(&aux, &aux2, dump, format_mode, "i[11:5]", COL_IMM1, 31, 25);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs2", COL_RS2, 24, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[4:0]", COL_IMM2, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;
        case FormatoB:
            g_str_copy(&aux2, g_mode_begin, "B ");
            muestra_campo(&aux, &aux2, dump, format_mode, "i[12]", COL_IMM1, 31, 31);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[10:5]", COL_IMM2, 30, 25);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs2", COL_RS2, 24, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[4:1]", COL_IMM3, 11, 8);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[11]", COL_IMM4, 7, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;
        case FormatoU:
            g_str_copy(&aux2, g_mode_begin, "U ");
            muestra_campo(&aux, &aux2, dump, format_mode, "imm", COL_IMM, 31, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "rs1", COL_RS1, 19, 15);
            muestra_campo(&aux, &aux2, dump, format_mode, "f3", COL_F3, 14, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            ...
            break;
        case FormatoJ:
            g_str_copy(&aux2, g_mode_begin, "J ");
            muestra_campo(&aux, &aux2, dump, format_mode, "i[20]", COL_IMM1, 31, 31);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[10:1]", COL_IMM4, 30, 21);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[11]", COL_IMM3, 20, 20);
            muestra_campo(&aux, &aux2, dump, format_mode, "i[19:12]", COL_IMM2, 19, 12);
            muestra_campo(&aux, &aux2, dump, format_mode, "rd", COL_RD, 11, 7);
            muestra_campo(&aux, &aux2, dump, format_mode, "op", COL_OP, 6, 0);
            break;

        default:
            muestra_campo(&aux, &aux2, dump, format_mode, "todo", "red", 31, 0);

    }

    if (format_mode == format_full) {
        g_str_printf(resultado, g_mode_begin, "%s %s",
                     g_str_string(&aux), g_str_string(&aux2));
    } else {
        g_str_copy(resultado, g_mode_begin, g_str_string(&aux2));
    }
}
...
void imprime_formato(
    ...
    if (PC == -1) {
        g_str_copy(formato, g_mode_begin, "-nop-");
    } else {
        if (en_region(mmem, PC, r_text)) {
            instr = lee_mem_instruc(mmem, PC);
            ...
            muestra_formato(&formato_campos, fmt, instr.valor, format_mode);

            g_str_printf(formato, g_mode_begin, F_XCODE " | %s",
                         (unsigned int) instr.valor, g_str_string(&formato_campos));

        } else {
            g_str_copy(formato, g_mode_begin, "unknown");
        }
    }
