...
void imprime_reg_txt(
    ...
    for (i = 0; i < TAM_REGISTROS; i++) {
        /*** Registros enteros */
        fprintf(stdout, "Regs[%2d]: %5" PRId64 "  ", i, Rint[i].valor.int_d);
    ...
    if (hay_double) {
        ...
        fprintf(stdout, "FPSR: %6" PRId64 "  ", Rint[TAM_REGISTROS].valor.int_d);
    }
    fprintf(stdout, "\n\n");
            ...
            if (i == next) {
                ...
                switch (tipo) {
                    ...
                    case t_byte:
                        ...
                        //sprintf(vstr, F_BYTE"(%c)", dato.int_d, dato.int_d);
                        sprintf(vstr, F_BYTE, (byte)(MAX_BYTE & dato.int_d));
                        if (dirty) {
                    ...
                    case t_half:
                        dato = mem_lee_region_datos(mmem, r, i, tipo, &dirty);
                        sprintf(vstr, F_HALF, (half)(MAX_HALF & dato.int_d));
                        if (dirty) {
                            fprintf(stdout, "|*%18s |", vstr);
                        } else {
                            fprintf(stdout, "|%19s |", vstr);
                        }
                    ...
                    case t_word:
                        dato = mem_lee_region_datos(mmem, r, i, tipo, &dirty);
                        sprintf(vstr, F_WORD, (word)(MAX_WORD & dato.int_d));
                        if (dirty) {
                            fprintf(stdout, "|*%36s |", vstr);
                        } else {
                            fprintf(stdout, "|%37s |", vstr);
                        }
