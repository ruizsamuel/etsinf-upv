 ...
 *
 * @brief
 *    Contiene las funciones para manejo de la tabla de símbolos
 *
...
static dword calcula_inmediato_referencia(
    ...
    dword offset;
    dword valor __attribute__((unused));
    dword offset11;
    ...
    switch (p_ref->parte_dir) {
        ...
        case DATA_REL_LO_12:
            ...
            if (offset > MAX_IMM) {
                fprintf(stderr, "Error. Desplazamiento para la referencia '%s' es mayor que %d (" F_DWORD ").\n",
                        p_ref->texto, (int)MAX_IMM, offset);
                fprintf(stderr, "       Instrucción con offset: " F_DWORD "\n", p_ref->offset);
...
void ts_simbolo_direccion(
    ...
    if (p_sim != NULL) {
        if (p_sim->direccion == direccion) {
            ...
            if (f_etiqueta) {
                sprintf(etiqueta, "%s + " F_DWORD, p_sim->texto, direccion - p_sim->direccion);
            } else {
