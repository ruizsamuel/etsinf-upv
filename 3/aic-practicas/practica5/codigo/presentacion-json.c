 ...
 *
 * @brief
 *    Contiene las funciones de presentación en formato JSON
 *
...
presentacion_interface_t presentacion_json = {
        ...
        .imprime_memoria_f = imprime_memoria_null, // Sin implementar
...
static void imprime_estado_selector_n_json
    ...
    if (e <= pwrtwo(CONFIG(predic.n_bits_selector) - 1) - 1) {
        sprintf(nom, "<font color=\"orange\">(%d) P1</font>", e);
    } else {
        sprintf(nom, "<font color=\"green\">(%d) P2</font>", e);
...
do { \
    ...
    CONFIG_ENTRY(0, "Número de vías Issue", int, CONFIG(num_vias.issue));
    CONFIG_ENTRY(1, "Número de buses WB", int, CONFIG(num_vias.bus));
    CONFIG_ENTRY(2, "Número vías Commit", int, CONFIG(num_vias.commit));
    CONFIG_ENTRY(3, "Tamaño ROB", int, CONFIG(tam_reorder));
    imprime_tipo_predictor(aux);
    CONFIG_ENTRY(4, "Tipo de predictor", string, aux);
    CONFIG_ENTRY(5, "Tamaño buffer predicción", int, CONFIG(predic.tam_buffer_predic));
...
#define OPERADOR(fila, operador, ident, estaciones) \
  OPER_ENTRY(fila, operador, CONFIG(ident.num_inicio), CONFIG(ident.teval), (CONFIG(ident.num_inicio) < CONFIG(ident.num_oper)) ? "Segmentado" : "Convencional", string, estaciones)

    sprintf(aux, "%d RS", CONFIG(tam_rs.enteros));
    OPERADOR(0, "Enteros", enteros, aux);
    sprintf(aux, "%d RS", CONFIG(tam_rs.sumrest));
    OPERADOR(1, "Suma/Resta C.F.", sumrest, aux);
    sprintf(aux, "%d RS", CONFIG(tam_rs.multdiv));
    OPERADOR(2, "Mult/Div. C.F.", multdiv, aux);
    sprintf(aux, "%d TL %d TE", CONFIG(tam_rs.carga), CONFIG(tam_rs.almacen));
...
static void imprime_estructuras_json() {
    ...
    jwObj_array("structures");
    {
        ...
        jwArr_object();
        {
            STRUCT_INFO(rob, "ROB", table, vertical, CONFIG(tam_reorder));
            jwObj_array("fields");
            {
                TABLE_FIELD(id, "Cod");
        ...
        jwArr_object();
        {
            STRUCT_INFO(rs, "RS", table, vertical,
        ...
        jwArr_object();
        {
            STRUCT_INFO(tl, "Buffer Lectura", table, vertical, CONFIG(tam_rs.carga));
            jwObj_array("fields");
            {
                TABLE_FIELD(id, "Cod");
                TABLE_FIELD(busy, "Ocupado");
        ...
        jwArr_object();
        {
            STRUCT_INFO(te, "Buffer Escritura", table, vertical, CONFIG(tam_rs.almacen));
            jwObj_array("fields");
            {
                TABLE_FIELD(id, "Cod");
                TABLE_FIELD(busy, "Ocupado");
        ...
        for (region_t r = (region_t) 0; r < num_regiones; r++) {
            ...
            jwArr_object();
            {
                // STRUCT_INFO(m[region], "Mem. Datos ([region]) ", table, vertical, tam_regiones(r));
                do {
                    char text[64];
            ...
            if (inst_store(RB[i].OP)) {
                ...
                if inst_jal_jalr(RB[i].OP) {
                    ...
                    if (RB[i].completado) {
                        ...
                        sprintf(text, "X" F_DWORD ",%s", RB[i].dest, str);
                    } else {
                        sprintf(text, "X" F_DWORD, RB[i].dest);
                    }
                ...
                sprintf(text, "%s" F_DWORD, (inst_to_Rint(RB[i].OP)) ? "X" : "F", RB[i].dest);
...
void muestra_datos_json(
    ...
    for (d = from; d < to; d += tam_elem) {
        ...
        tipo_dato = mmem->datos[r].tipos[dir_a_offset(mmem, d, r)];
...
(
    ...
    if ((CONFIG(predic.tipo_predictor) == CLARIVIDENTE) || (CONFIG(predic.tipo_predictor) == BTB_1_BIT) ||
        ...
        if ((CONFIG(predic.tipo_predictor) != GLOBAL_n_BIT_S) && (CONFIG(predic.tipo_predictor) != GLOBAL_n_BIT_H)) {
        ...
        if (CONFIG(predic.tipo_predictor) == HYBRID_n_BIT_S) {
        ...
        for (i = 0; i < TAM_BUFFER_PREDIC; i++) {
            ...
            if (BTB[i].orden > 0) { /* Entrada ocupada */
                ...
                if ((CONFIG(predic.tipo_predictor) != GLOBAL_n_BIT_S) && (CONFIG(predic.tipo_predictor) != GLOBAL_n_BIT_H)) {
                    for (j = 0; j < pwrtwo(n_bits_branch_mask_local); j++) {
                        imprime_estado_predictor_n_json(aux, BTB[i].v_estado[j], CONFIG(predic.n_bits_pred_local));
        ...
        if ((CONFIG(predic.tipo_predictor) == GLOBAL_n_BIT_H) || (CONFIG(predic.tipo_predictor) == GLOBAL_n_BIT_S) ||
