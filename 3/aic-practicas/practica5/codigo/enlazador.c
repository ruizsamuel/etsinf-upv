 ...
 *
 * @brief
 *    Contiene las funciones necesarias para enlazar los segmentos de
...
void inicializa_programa(
    ...
    for (int i = 0; i < num_ficheros; ++i) {
        inicializa_objeto(&prog->l_objs[i], ficheros_entrada[i]);
    }
...
void inicializa_objeto(
    ...
    memset(obj, 0, sizeof(fichero_objeto_t));
...
void enlazador(
    ...
    for (r = (region_t) 0; r < num_regiones; ++r) {
        switch (r) {
            case r_reserved:
                ...
                dir += TAM_MEM_RESERVADA;

                mm->regiones[r].inicio = 0;
                mm->regiones[r].final = TAM_MEM_RESERVADA;
                break;
            case r_text:
            case r_data:
                ...
                for (int i = 0; i < prog->n_objs; ++i) {
                     ...
                     * considerando el alineamiento requerido por cada una.
                     */
    ...
    for (int i = 0; i < prog->n_objs; ++i) {
        ...
        // Resuelve las direcciones locales para cada objeto
        ts_resuelve_direcciones_locales(&obj->ts, &obj->memoria);
    ...
    for (int i = 0; i < prog->n_objs; ++i) {
        ...
        ts_resuelve_referencias_globales(mm, prog->simbolos, &obj->ts, &obj->memoria);

        // Resuelve las referencias globales para cada objeto
        ts_resuelve_direcciones_globales(prog->simbolos, &obj->ts, &obj->memoria);
...
void concatena_objetos(
    ...
    for (r = (region_t) 0; r < num_regiones; ++r) {
        for (int i = 0; i < prog->n_objs; ++i) {
            ...
            switch (r) {
                case r_text:
                    ind_prog = indice_code(dir_a_offset(&prog->memoria, dir, r));
                    ind_obj = indice_code(dir_a_offset(&obj->memoria, dir, r));
                ...
                case r_stack:
                    ...
                    memcpy(&(prog->memoria.datos[r].mem.m_byte[ind_prog]),
                           &(obj->memoria.datos[r].mem.m_byte[ind_obj]), r_tam);
                    // Hay un tipo por cada byte, así que se puede utilizar el mismo índice
                    memcpy(&(prog->memoria.datos[r].tipos[ind_prog]),
                           &(obj->memoria.datos[r].tipos[ind_obj]), r_tam * sizeof(tipo_t));
                    break;
                default:
                    // Nada que copiar
                    break;
