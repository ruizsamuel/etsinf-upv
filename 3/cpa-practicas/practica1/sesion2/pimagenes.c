           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {
    /*
     * Paralelizar este bucle (o el siguiente más interno (j) ) es lo más eficiente, pues minimiza las dependencias
     * entre hilos. Cada hilo puede trabajar de forma independiente (cada fila de la imagen).
     */
    #pragma omp parallel for private(j, resultado, k, l, tot, v)
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        resultado.r = 0;
        resultado.g = 0;
...
int main()
{
  ...
  printf("El tiempo total de filtrado ha sido de %f segundos\n", t2-t1);
